 ...
 * so their handlers will be enter the safe accessing mode for ui object.
 ...
 *
 */
public class TimeEventHandler extends Handler {
	private static final int HANDLE_REFRESH_TIME = 100;
	...
	public synchronized void removeAllMessages() {
		removeMessages(TimeEventHandler.MESSAGE_ID);
	}
	...
	public synchronized void removeMessage(Object msgId) {
		...
		removeMessages(TimeEventHandler.MESSAGE_ID, msgId);
	}
				...
				if (eventData.listener != null) {
					...
					if (eventType == HANDLE_TIME_EVENT) {
						...
						synchronized (eventData.observer) {
							eventData.listener.onTimeEvent(eventData.count);
						}
					}

					// Add time event
					sendMessageDelayed(msg, eventData.delay);
					msgAdded = true;
				}
			}
		} catch (Exception e) {
